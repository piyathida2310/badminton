...
export default function MatchTable({ tournamentId }: MatchTableProps) {
  ...
  const renderStatusBadge = (status: string) => {
    ...
    if (status === "แข่งสำเร็จ")
      return (
        <span
          className={`${base} border-[#2ED3B7]/20 bg-[#2ED3B7]/20 text-[#194185]`}
        >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-3 mb-5 px-2">
        ...
        <div className="relative w-48 sm:w-56">
          <select
            ...
            focus:ring-2 focus:ring-[#194185]/20 focus:border-[#194185] transition-all cursor-pointer"
          >
      ...
      <div className="hidden sm:block overflow-x-auto">
        <div className="inline-block min-w-full overflow-hidden rounded-xl border border-[#194185]/10 shadow-lg bg-white/80 backdrop-blur-md">
          <table className="min-w-full text-xs text-center border border-gray-300 border-collapse">
            <thead className="bg-[#194185]/10 text-[#194185]">
              <tr>
                {[
                  mh.colMatch,
                  mh.colCategory,
                  ...
                  <th
                    key={i}
                    className="px-2 py-2 font-semibold border border-[#2ED3B7]/20 whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {filteredMatches.map((m, i) => (
                <tr
                  key={`${m.stage}-${m.id}`}
                  className={`transition-all duration-150 hover:bg-[#2ED3B7]/5 ${i % 2 === 0 ? "bg-white" : "bg-[#2ED3B7]/5"
                    }`}
                >
                  <td className="p-2 border border-[#2ED3B7]/20 font-bold text-[#194185] whitespace-nowrap">
                    {m.displayId || m.id}
                  </td>
                  <td className="p-2 border border-[#2ED3B7]/20">{m.type}</td>
                  <td className="p-2 border border-[#2ED3B7]/20">{m.round}</td>
                  <td className="p-2 border border-[#2ED3B7]/20">
                    {renderStatusBadge(m.status)}
                  </td>
                  <td className="p-2 border border-[#2ED3B7]/20">{m.timeIn}</td>
                  <td className="p-2 border border-[#2ED3B7]/20">
                    {m.group.replace(/P_PLUS/g, "P+").replace(/P_MINUS/g, "P-")}
                  </td>
                  <td className="p-2 border border-[#2ED3B7]/20 font-medium">
                    {m.team1}
                  </td>
                  <td className="p-2 border border-[#2ED3B7]/20">
                    {renderPlayers(m.matchType, m.player1A, m.player1B)}
                  </td>
                  <td className="p-2 border border-[#2ED3B7]/20 font-bold text-[#194185]">
                    {m.score || "-"}
                  </td>
                  <td className="p-2 border border-[#2ED3B7]/20 font-medium">
                    {m.team2}
