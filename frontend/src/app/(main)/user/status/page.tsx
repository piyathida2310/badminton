...
export default function StatusPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#E6F8F3] via-[#DDEDFC] to-[#F9F9FF] py-10 px-6 text-[#2F3E46]">
      <h1 className="text-center text-4xl font-bold mb-10 text-[#1E293B] drop-shadow-sm">
        สถานะผู้เข้าแข่งขัน
      </h1>
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        {filteredTeams.map((team) =>
          team.members.length > 0 ? (
            <div
              key={team.teamName}
              className="overflow-x-auto rounded-2xl shadow-lg border border-slate-200 bg-gradient-to-br from-[#FFFFFF] to-[#E6F3F9] backdrop-blur-sm"
            >
              <div className="bg-gradient-to-r from-[#5CD6C0] to-[#6BA8F8] py-3 text-center text-lg font-semibold text-white rounded-t-2xl shadow-inner tracking-wide">
                ทีม {team.teamName}
              </div>
      ...
      {showPayment && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-8 max-w-2xl w-[90%] relative border-2 border-sky-200">
            <button
              onClick={() => setShowPayment(false)}
              className="absolute top-3 right-4 text-gray-500 hover:text-gray-700"
            ></button>
            <h2 className="text-xl font-bold text-center mb-6 text-[#1E293B]">
              ช่องทางการชำระเงิน
            </h2>

            <div className="grid md:grid-cols-2 gap-6 items-center justify-center">
              {/* QR Code */}
              <div className="bg-[#F0F9FF] border rounded-xl p-6 flex flex-col items-center justify-center shadow-sm">
                <img
              ...
              {/* Upload Slip */}
              <div className="bg-[#F0F9FF] border rounded-xl p-6 flex flex-col items-center justify-center shadow-sm h-full min-h-[300px]">
                {uploadedSlip ? (
                  <img
                    src={uploadedSlip}
                    alt="slip"
                  ...
                  <label className="flex flex-col items-center justify-center text-gray-500 cursor-pointer">
                    ...
                    <p className="text-sm">อัปโหลดสลิปชำระเงิน</p>
                    <input
                      type="file"
                      accept="image/*"
            ...
            <div className="text-center mt-6">
              <button
                ...
                className="bg-gradient-to-r from-[#6BA8F8] to-[#5CD6C0] hover:opacity-90 text-white font-semibold px-6 py-2 rounded-lg shadow-md"
              >
                เสร็จสิ้น
              </button>
            </div>
