    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          errorMessage: {
            type: "string",
            description: "สำคัญมาก: หากรายชื่อผู้สมัครไม่ตรงกับจำนวน/สเปกที่ผู้ใช้สั่ง 'อย่างชัดเจน' (เช่น ขออายุ 45 และ 55 แต่ไม่มีเลยทั้งคู่) ให้ตรวจเช็คและรวบรวมสาเหตุที่จัดไม่ได้ 'ทุกข้อ' มาบอกรวมกัน เป็นภาษาไทยในช่องนี้ เช่น 'ไม่สามารถจัดได้เนื่องจากไม่มีผู้เล่นอายุ 45 ปี และไม่มีผู้เล่นอายุ 55 ปี' (ถ้าระบุเงื่อนไขไหนไม่ได้ให้บอกให้หมด ถ้าไม่มีปัญหาเลยให้ปล่อยว่างไว้)",
          },
          groups: {
            type: "object",
...
STEP 2 — จัดกลุ่มที่มี constraint เจาะจงก่อน (เช่น กลุ่มที่มีสั่งไว้พิเศษ ไม่ว่าจะ A, B หรือ H):
  ...
  → ถ้ารู้สึกว่าคน 'ตรงสเปกเป๊ะๆ' หมดแล้ว หรือหาไม่ได้เลยจริงๆ จนต้องทำ ID ซ้ำ:
     หยุดทันที! แล้วไปเขียนคำอธิบายปัญหาลงในช่อง errorMessage แทน (ระบุให้ครบทุกเงื่อนไขที่หาไม่ได้ เช่น 'ไม่มีอายุ 45 และไม่มีอายุ 55') ระบบจะส่งข้อความนี้ไปแจ้งผู้ใช้เอง
  → กฎเหล็กสูงสุด: ห้ามใช้ ID ซ้ำเด็ดขาด ถ้าถูกใช้ไปแล้วห้ามใช้ซ้ำอีก
...
- DOUBLE tournament: Age จะเป็นอายุเฉลี่ยของผู้เล่น 2 คน
 ...
 คำเตือนสุดท้าย: ถ้าคุณจัดกลุ่มแล้วเจอ Error ว่า "หา ID ไม่ครบ / ซ้ำ / เกิน 4" แปลว่าคุณบังคับเงื่อนไขคุณลักษณะ (อายุ, เพศ) ไว้โหดเกินไป ให้คุณยอมละทิ้งเงื่อนไขนั้นแล้วหยิบ ID ไหนก็ได้ที่ยังว่างอยู่มาใส่ให้เต็มกล่อง เพื่อให้โปรแกรมทำงานต่อได้!
`.trim();
